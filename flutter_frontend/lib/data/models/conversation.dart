...
class Conversation {
  ...
  factory Conversation.fromMap(Map<String, dynamic> json) => Conversation(
        id: json["_id"],
        listUserIn: List<User>.from(
          (json["userIns"] as List<dynamic>)
              .map((e) => User.fromMap(e["userIn"])),
        ).toList(),
  ...
  factory Conversation.fromMapRoom(Map<String, dynamic> json) => Conversation(
        id: json["_id"],
        name: json["name"],
        avatarRoom: (json["avatar"] == null || json["avatar"] == "")
            ? ImagePath.avatarChatGroup
            : json["avatar"],
        listUserIn: List<User>.from(
          (json["members"] as List<dynamic>).map((e) => User.fromMapRoom(e)),
        ).toList(),
        listMessage: List<Message>.from(
          (json["list_message"] as List<dynamic>)
              .map((e) => Message.fromMap(e)),
        ).toList(),
        isRoom: true,
